...
def create_board_h(board_name, target_dir, linkspeed=None):
    ...
    with open(path, "w", encoding="utf-8") as fd:
        ...
        if linkspeed is not None:
            # Per-port line rate of this target (from data.json linkspeed,
            # e.g. 100 or 40): selects the MAC bring-up config and the
            # Si5328 GT refclk plan.
            fd.write(f"#define LINE_RATE {int(str(linkspeed))}\n")
        fd.write("#endif\n")
...
def main():
    ...
    if data_json_path:
        ...
        linkspeed_for_header = design.get("linkspeed")
